...
python user_simulation_math_tutoring.py --version=zero-shot
echo ""
...
echo "========== USER PROFILE: KNOWLEDGE STATE =========="
echo "Generation based on user's knowledge state"
echo ""
for i in {1..4}; do
    echo "Run $i: User profile with knowledge state"
    python user_simulation_math_tutoring.py \
        --version=zero-shot-cot-user-profile \
        --user_profile_version=knowledge_state
    echo ""
done

echo "========== USER PROFILE: COMBINED WRITING & INTERACTION & KNOWLEDGE STATE =========="
echo "Generation combining writing style and interaction patterns and knowledge state"
echo ""
echo "Run: User profile with combined writing and interaction style and knowledge state"
python user_simulation_math_tutoring.py \
    --version=zero-shot-cot-user-profile \
    --user_profile_version=knowledge_state_and_writing_interaction_style
echo ""
...
echo "========== ADVANCED: KNOWLEDGE STATE + LENGTH CONTROL =========="
echo "User knowledge state with length constraints (currently active)"
echo ""
for i in {1..1}; do
...
echo "========== ADVANCED: KNOWLEDGE STATE + COMBINED STYLE + LENGTH CONTROL =========="
echo "Full combination of knowledge state, styles, and length constraints"
echo ""
for i in {1..3}; do
