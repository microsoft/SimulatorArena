...
# ============================================================================
# User Simulator Evaluation Script for Document Creation
# ============================================================================
# This script evaluates the performance of user simulators for document creation
# by measuring correlation with human annotations at multiple levels
# 
...
# Function to print substep headers
print_substep() {
    echo ""
    echo -e "${MAGENTA}------------------------------------------------------------${NC}"
    echo -e "${MAGENTA}$1${NC}"
...
main() {
    print_step "EVALUATING USER SIMULATORS: $FILE_NAME"
    
    # Change to scripts directory for running Python scripts
    cd scripts
    
    # Check for human annotations
    ANNOTATION_FILE="../../../simulation/data/${ANNOTATION_ID}.json"
    if ! check_file "$ANNOTATION_FILE"; then
        echo -e "${RED}Error: Human annotation file not found${NC}"
        echo "Expected location: $ANNOTATION_FILE"
        exit 1
    fi
    ...
    # ============================================================
    # STEP 4: Simulator Performance Analysis
    # ============================================================
    print_step "STEP 4: Simulator Performance Analysis"
    
    print_substep "4.1: Document Aspect Correlation"
    
    # Analyze document aspect
    if [ "$NORMALIZE" = true ]; then
        python show_simulator_performance.py \
            --file_name "$FILE_NAME" \
            --annotation_id "$ANNOTATION_ID" \
            --aspect "document" \
            --normalize
    ...
    print_substep "4.2: Interaction Aspect Correlation"
    
    # Analyze interaction aspect
    if [ "$NORMALIZE" = true ]; then
        python show_simulator_performance.py \
            --file_name "$FILE_NAME" \
            --annotation_id "$ANNOTATION_ID" \
    ...
    print_substep "4.3: Combined Analysis"
    
    # Analyze both aspects together
    if [ "$NORMALIZE" = true ]; then
        python show_simulator_performance.py \
            --file_name "$FILE_NAME" \
            --annotation_id "$ANNOTATION_ID" \
            --aspect "both" \
            --normalize \
            --export "../evaluation_outputs/simulator_performance_${FILE_NAME}.json"
    else
        python show_simulator_performance.py \
            --file_name "$FILE_NAME" \
            --annotation_id "$ANNOTATION_ID" \
    ...
    # ============================================================
    # STEP 5: Assistant Performance Summary
    # ============================================================
    print_step "STEP 5: Assistant Performance Summary"
    
    echo "Generating assistant performance summary..."
    ...
    echo "  - Simulator performance: evaluation_outputs/simulator_performance_${FILE_NAME}.json"
    echo ""
    echo -e "${MAGENTA}Key Metrics:${NC}"
    echo "  - Instance-level: Individual conversation correlations"
    echo "  - Intermediate-level: Grouped by (model, document_type)"
    echo "  - System-level: Grouped by model only"
    echo ""
    echo "Higher correlations indicate better simulator performance!"
}
